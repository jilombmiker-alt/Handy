...
const MIRROR_GALLERY_MAX_ITEMS = 12;
const MIRROR_GALLERY_ID_PATTERN = /^[a-f\d]{8}-[a-f\d]{4}-[1-8][a-f\d]{3}-[89ab][a-f\d]{3}-[a-f\d]{12}$/i;

function normalizeMirrorGallery(value) {
  ...
  const items = (Array.isArray(source.items) ? source.items : [])
    .map((item) => {
      ...
      if (!MIRROR_GALLERY_ID_PATTERN.test(id) || seen.has(id)) return null;
      const fileName = String(item.fileName || '').trim();
      if (fileName !== `mirror-${id}.jpg`) return null;
      const createdAt = Number(item.createdAt);
      seen.add(id);
      return {
        id,
        fileName,
        createdAt: Number.isFinite(createdAt) ? Math.max(0, createdAt) : Date.now(),
      };
    })
    .filter(Boolean)
    .slice(0, MIRROR_GALLERY_MAX_ITEMS);
  const requestedActiveId = String(source.activeId || '').trim().toLowerCase();
  return {
    version: 1,
    activeId: items.some((item) => item.id === requestedActiveId)
      ? requestedActiveId
      : (items[0]?.id || ''),
    items,
  };
...
function normalizeWindowRows(rows, { preserveDuplicates = false } = {}) {
  ...
  return rows
    ...
    .filter((item, index, list) => list.findIndex(
      (other) => preserveDuplicates ? other.id === item.id : other.pid === item.pid && other.title === item.title
    ) === index);
}
...
function normalizeCredentialInput(value, id, createdAt) {
  ...
  if (!service || !password) return null;
  let website='';
  if(value.website){try{const url=new URL(String(value.website));if(!['https:','http:'].includes(url.protocol)||url.username||url.password||url.href.length>2000)return null;website=url.href;}catch{return null;}}
  return {
    ...
    password,
    ...(website?{website}:{}),
    ...(value.note?{note:String(value.note).slice(0,2000)}:{}),
...
// 面板展开和拖拽都会明显卡顿。
// 用户启用后才注册独立快捷键，默认 Option+V，具体配置由 clipboard runtime 管理。
function clipboardServicePolicy(features) {
  ...
  return {
    ...
    registerGlobalShortcut: source.clip === true,
  };
}

const CONFIGURABLE_FEATURES = new Set(['inbox', 'mail', 'todo', 'notes', 'links', 'recordings', 'credentials', 'clip']);
...
// 系统级快捷键不能是裸键：Space / Shift+Space 会直接抢走输入、网页滚动和
// 应用自己的操作。至少要有 Command / Control / Option 之一，Shift 只能作为附加修饰键。
const DEFAULT_PANEL_SHORTCUT = 'CommandOrControl+Shift+Space';

const RESERVED_PANEL_SHORTCUTS = new Set([
  'Command+Space',
  'Control+Space',
  'Command+Tab',
  'Command+Q',
...
function panelShortcutIdentity(shortcut) {
  if (typeof shortcut !== 'string') return '';
  const parts = shortcut.split('+').filter(Boolean);
  const key = parts.pop();
  if (!key) return '';
  const present = new Set(parts.map((part) => part === 'CommandOrControl' ? 'Command' : part === 'Option' ? 'Alt' : part));
  const ordered = ['Command', 'Control', 'Alt', 'Shift'].filter((part) => present.has(part));
  return [...ordered, key].join('+');
}

function isReservedPanelShortcut(shortcut) {
  return RESERVED_PANEL_SHORTCUTS.has(panelShortcutIdentity(shortcut));
}

function isSafePanelShortcut(shortcut) {
  if (shortcut === 'LeftOption+LeftCommand' || /^F(?:[1-9]|1[0-9]|2[0-4])$/.test(shortcut)) return true;
  ...
  const conflictSafeModifiers = new Set(['CommandOrControl', 'Command', 'Control', 'Alt', 'Option']);
  return tokens.every((token) => modifiers.has(token))
    && tokens.some((token) => conflictSafeModifiers.has(token))
    && /^(?:[A-Z0-9]|F(?:[1-9]|1[0-9]|2[0-4])|Space|Tab|Escape|Left|Right|Up|Down|Home|End|PageUp|PageDown|Backspace|Delete|Enter)$/.test(key)
    && !isReservedPanelShortcut(shortcut);
}

function normalizePanelShortcut(shortcut) {
  return isSafePanelShortcut(shortcut) ? shortcut : DEFAULT_PANEL_SHORTCUT;
}

// These are toggle/navigation commands, not a source of playback metadata.
// Never infer playback state from successful event delivery.
function sodaShortcutSpec(action) {
  if (['toggle', 'play', 'pause'].includes(action)) return { keyCode: 49, command: false, dismissOverlays: false };
  if (action === 'next') return { keyCode: 124, command: true, dismissOverlays: false };
  if (action === 'previous') return { keyCode: 123, command: true, dismissOverlays: false };
  return null;
}

async function controlSodaMusic(action, dependencies = {}) {
  if (!['toggle', 'play', 'pause', 'next', 'previous'].includes(action)) {
  ...
  if (!running) {
    if (!['play', 'toggle'].includes(action)) return { ok: false, error: 'no_active_session', running: false, playing: false };
  ...
  return { ok: true, running: true, playing: null, commandSent: true, bootstrapped };
}
