(function exposeNotchDomain(root, factory) {
  ...
  function clipboardEntryContentKey(entry) {
    if (!entry || typeof entry !== 'object') return '';
    if (typeof entry.contentKey === 'string' && /^(?:text|image|file):[a-f0-9]{64}$/i.test(entry.contentKey)) {
      return entry.contentKey.toLowerCase();
    }
    // 文本旧数据可以按完整内容合并；图片必须等主进程计算字节哈希。
    if ((entry.type === 'text' || entry.type === 'url') && typeof entry.text === 'string') {
      return `${entry.type}:${entry.text}`;
    }
    return '';
  }

  function compactClipboardHistory(history, maxEntries = 100) {
    ...
    for (const raw of Array.isArray(history) ? history : []) {
      ...
      const entry = { ...raw, repeatCount: Math.max(1, Math.floor(Number(raw.repeatCount) || 1)) };
      const key = clipboardEntryContentKey(entry);
      const existing = key ? seen.get(key) : null;
      if (existing) {
        existing.repeatCount += entry.repeatCount;
        evicted.push(entry);
        if (entry.id && existing.id && entry.id !== existing.id) {
          replacements.push({ fromId: entry.id, toId: existing.id });
        }
        continue;
      }
      kept.push(entry);
    ...
    evicted.push(...kept.slice(limit));
    return { history: kept.slice(0, limit), evicted, replacements };
  }

  function prependClipboardHistory(history, entry, maxEntries = 100) {
    const source = Array.isArray(history) ? history : [];
    const key = clipboardEntryContentKey(entry);
    const existing = key ? source.find((item) => clipboardEntryContentKey(item) === key) : null;
    let head = entry;
    const discardedIncoming = [];
    if (existing) {
      head = {
        ...existing,
        ...entry,
        id: existing.id,
        imagePath: existing.imagePath || entry.imagePath,
        repeatCount: Math.max(1, Math.floor(Number(existing.repeatCount) || 1)) + 1,
      };
      if (entry.type === 'image' && entry.imagePath && entry.imagePath !== head.imagePath) {
        discardedIncoming.push(entry);
      }
    }
    const withoutExisting = existing ? source.filter((item) => item !== existing) : source;
    const compacted = compactClipboardHistory([head, ...withoutExisting], maxEntries);
    return { ...compacted, evicted: [...discardedIncoming, ...compacted.evicted] };
  }
  ...
  function createRecording(value) {
    ...
    return {
      ...
      ...(value.quickCapture===true?{quickCapture:true}:{}),
      ...(value.voice && typeof value.voice==='object' ? {voice:(typeof module==='object'&&module.exports?require('./voice-memo-model'):globalThis.VoiceMemoModel).normalize(value.voice)} : {}),
    };
  }
  ...
  function normalizeNoteArchive(value) {
    ...
    return value
      .map((item) => {
        ...
        const extra = {};
        for(const key of ['sourceQuickRecordId','sourceRecordingId'])if(typeof item[key]==='string'&&item[key].length<=200)extra[key]=item[key];
        if(typeof item.sourceClipboardKey==='string'&&/^[a-f0-9]{64}$/.test(item.sourceClipboardKey))extra.sourceClipboardKey=item.sourceClipboardKey;
        // Optional meeting data belongs to this note; old archives remain unchanged.
        if (item.meeting && typeof item.meeting === 'object' && !Array.isArray(item.meeting)) extra.meeting = item.meeting;
        return { id, title, titleSource, content, createdAt, updatedAt, ...extra };
      })
      .filter(Boolean)
  ...
  function settingsSummary(input = {}) {
    ...
    return {
      shortcut: String(appSettings.shortcut || 'CommandOrControl+Shift+Space'),
      shortcutLabel: panelShortcutLabel(appSettings.shortcut || 'CommandOrControl+Shift+Space'),
  ...
  function panelShortcutLabel(shortcut) {
    if (shortcut === 'LeftOption+LeftCommand') return '左 Option + 左 Command（松开触发）';
  ...
  function shortcutConflictAssessment(shortcut) {
    const identity = String(shortcut || '')
      .replace('CommandOrControl', 'Command')
      .replace('Option', 'Alt');
    const reserved = new Map([
      ['Command+Space', 'macOS 通常用它打开聚焦搜索'],
      ['Control+Space', '常用于切换输入法'],
      ['Command+Tab', 'macOS 用它切换应用'],
      ['Command+Q', '大多数应用用它退出'],
      ['Command+W', '大多数应用用它关闭窗口'],
      ['Command+H', 'macOS 用它隐藏应用'],
      ['Command+M', 'macOS 用它最小化窗口'],
      ['Command+Alt+Escape', 'macOS 用它打开强制退出'],
      ['Command+Alt+Space', 'macOS 可能用它打开 Finder 搜索'],
      ['Command+Control+Q', 'macOS 用它锁定屏幕'],
      ['Command+Control+Space', 'macOS 用它打开表情与符号'],
      ['Command+Shift+3', 'macOS 用它截取整个屏幕'],
      ['Command+Shift+4', 'macOS 用它截取选定区域'],
      ['Command+Shift+5', 'macOS 用它打开截图工具'],
      ['Control+Left', 'macOS 可能用它切换桌面'],
      ['Control+Right', 'macOS 可能用它切换桌面'],
      ['Control+Up', 'macOS 可能用它打开调度中心'],
      ['Control+Down', 'macOS 可能用它显示应用窗口'],
    ]);
    if (reserved.has(identity)) {
      return { state: 'blocked', message: `${reserved.get(identity)}，请换一组。` };
    }
    return {
      state: 'ready',
      message: '未发现常见系统冲突。下一步会让你在常用应用里亲自试一次。',
    };
  }
