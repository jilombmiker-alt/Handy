(() => {
  ...
  window.SimpleTimer={format,mount(root,api,{compact=false,floating=false,open=()=>{}}={}){
    ...
    const progress=node('progress',null,'timer-progress');progress.max=1;progress.value=0;progress.setAttribute('aria-label','计时进度');progress.hidden=true;
    if(floating){options.prepend(finish);more.textContent='⋯';more.setAttribute('aria-label','更多计时操作');}
    box.append(head,digits,progress,actions,status,options);root.replaceChildren(box);box.classList.toggle('timer-compact',compact);
    ...
    function render(s){if(s?.ok&&state?.ok&&s.revision<state.revision)return;const wasActive=!!state?.active;state=s;if(!s?.ok){say(errors[s?.error]||'读取失败，请稍后重试。',true);controls();return;}
      const a=s.active;if(a){mode.value=a.mode;title.value=a.title;const overtime=a.mode==='countdown'&&a.currentMs>=a.plannedMs;clock.textContent=format(a.mode==='countup'?a.currentMs:Math.ceil(Math.max(0,a.plannedMs-a.currentMs)/1000)*1000);label.textContent=overtime?'时间到':a.mode==='countup'?'已用时间':'剩余时间';progress.hidden=a.mode!=='countdown';progress.value=a.plannedMs?Math.min(1,a.currentMs/a.plannedMs):0;if(floating)mode.hidden=true;say(`${a.title||''}${a.recovered?' · 上次计时已暂停':!a.running?' · 已暂停':''}`.replace(/^ · /,''));}
      ...
      if(!a){label.textContent='计时';progress.hidden=true;mode.hidden=false;}
      controls();suggest();
    }
