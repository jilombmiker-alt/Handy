...
test('clipboard history moves repeated content to the front instead of duplicating it', () => {
  ...
  assert.deepEqual(result.history.map((entry) => entry.id), ['first']);
  assert.equal(result.history[0].timestamp, 200);
  assert.equal(result.history[0].repeatCount, 2);
  assert.deepEqual(result.evicted, []);
});

test('panel shortcut labels explain the physical keys in plain language', () => {
  assert.equal(panelShortcutLabel('CommandOrControl+Shift+Space'), 'Command + Shift + 空格键');
  assert.equal(panelShortcutLabel('Control+Option+7'), 'Control + Option + 7');
});

test('shortcut conflict assessment blocks common macOS and app commands', () => {
  assert.equal(shortcutConflictAssessment('Command+Space').state, 'blocked');
  assert.equal(shortcutConflictAssessment('Command+W').state, 'blocked');
  assert.equal(shortcutConflictAssessment('Command+Shift+4').state, 'blocked');
  assert.deepEqual(shortcutConflictAssessment('Alt+Shift+Space'), {
    state: 'ready',
    message: '未发现常见系统冲突。下一步会让你在常用应用里亲自试一次。',
  });
});

test('clipboard history compacts exact image hashes and exposes favorite id replacements', () => {
  const hash = `image:${'a'.repeat(64)}`;
  const result = compactClipboardHistory([
    { id: 'newer', type: 'image', imagePath: '/tmp/newer.png', contentKey: hash, timestamp: 200 },
    { id: 'older', type: 'image', imagePath: '/tmp/older.png', contentKey: hash, timestamp: 100 },
  ]);
  assert.deepEqual(result.history.map((entry) => entry.id), ['newer']);
  assert.equal(result.history[0].repeatCount, 2);
  assert.deepEqual(result.evicted.map((entry) => entry.id), ['older']);
  assert.deepEqual(result.replacements, [{ fromId: 'older', toId: 'newer' }]);
});
...
test('clipboard history evicts only entries beyond its capacity', () => {
  ...
  const result = prependClipboardHistory(
    previous,
    { id: 'new', type: 'text', text: 'B', timestamp: 200 },
    2
  );
...
test('settings stays at the far right when optional tabs are hidden', () => {
  ...
  const tabs = ['home', 'inbox', 'todo', 'notes', 'links', 'recordings', 'credentials', 'clip', 'settings'];
  assert.deepEqual(visiblePanelTabs(tabs, { todo: false, clip: true }), [
    'home', 'inbox', 'notes', 'links', 'recordings', 'credentials', 'clip', 'settings',
  ]);
